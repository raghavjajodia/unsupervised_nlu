...
def latent_loss(outputs, target, device, padtoken=2, unktoken=3):
    ...
    taglogitloss = torch.stack(taglogitloss).view(btchSize, sentLen, numtags)
    wordlogitloss = torch.stack(wordlogitloss).view(btchSize, sentLen, numtags)
    
    totalloss = taglogitloss + wordlogitloss
    
    #mask the tags if output word is not present in tag's vocab
    outofvocab_mask = (target.view(btchSize, sentLen, numtags) == unktoken)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_num, (inputs, target, labels) in enumerate(dataloaders["train"]):
            ...
            with torch.set_grad_enabled(True):
                ...
                loss, useful_tokens = criterion(outputs, target, device, padtoken)
...
def evaluate(model, criterion, device, validation_loader, padtoken):  
    ...
    for batch_num, (inputs, targets, labels) in enumerate(validation_loader):
        ...
        outputs = model(inputs)
        loss, useful_tokens = criterion(outputs, targets, device, padtoken)
...
stepsize = 5
epochs = 30
outfolder = '/data/rj1408/ptb_wsj_pos/models/basic_imp0/a/'
