...
class LM_latent(nn.Module):
    def __init__(self,vocab_size,hidden_size,token_embedding_size,tag_embedding_size,tags,mask,device, lstm_layers=1):
        ...
        self.tag_projections = nn.ModuleList([nn.Linear(self.hidden_size,self.vocab_size) for i in range(self.num_tags)])
    ...
    def forward(self,input_seq):
        ...
        word_distributions_logits = torch.stack([self.tag_projections[i](h) for i in range(self.num_tags)],dim=2) #batch_size, sent_len, num_tags, vocab_size
